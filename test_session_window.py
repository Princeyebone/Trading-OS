...
def run_pure_session_system():
    ...
    # Test London session only (07:00 to 14:00 UTC) vs Full Session (07:00 to 18:00)
    for session_window in [(7, 13), (7, 15), (8, 14), (7, 18)]:
        ...
        for i in range(25, n - 20):
            ...
            if locked_green or i <= last_exit or not (session_window[0] <= cur_hour <= session_window[1]):
                continue

            ladder = [0.03, 0.07, 0.16, 0.25]
            cur_lot = ladder[min(consec_losses, len(ladder)-1)]
            rsi_low = 28 - (consec_losses * 2)
            rsi_high = 72 + (consec_losses * 2)

            long_c = (c[i-1] < lower[i-1]) and (rsi[i-1] < rsi_low) and (c[i-1] >= h1_ema[i-1] - 0.20)
            short_c = (c[i-1] > upper[i-1]) and (rsi[i-1] > rsi_high) and (c[i-1] <= h1_ema[i-1] + 0.20)
        ...
        print(f"SESSION WINDOW: {session_window[0]}:00 - {session_window[1]}:00")
