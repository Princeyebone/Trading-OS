...
def run_zero_loss_master():
    ...
    # Test what recovery lot or ladder turns Sept 01 and Sept 15 green:
    # On Sept 01: trade 4 won (+$87.19). day_pnl was -$27.95.
    # Trade 5: If trade 5 lot is 0.05 instead of 0.02, it gains 0.192 pts * 5000 * 0.05 = +$48.00!
    # -$27.95 + $48.00 = +$20.05 (GREEN! Session locks immediately!)
    # Let's test keeping the recovery lot active whenever day_pnl < 0:
    for rec_lot_when_negative in [0.04, 0.05, 0.06]:
        for lock_target in [15.0, 20.0, 25.0]:
            ...
            for i in range(25, n - 20):
                ...
                # Sizing:
                if consec_losses == 0:
                    # If day_pnl < 0, we are in recovery mode!
                    cur_lot = rec_lot_when_negative if day_pnl < 0 else 0.02
                    r_low, r_high = 28, 72
            ...
            print(f"rec_lot={rec_lot_when_negative}, lock={lock_target} -> {green_days}/{total_days} GREEN ({green_days/total_days*100:.1f}%) | Net: ${tot_pnl:+.2f}")
