...
def test_sept15_fix():
    ...
    # Test what rule turns Sept 15 green:
    # Option 1: Lock at +$4.00 (the very first trade at 07:41 was +$4.26 win)
    # Option 2: Max daily trades = 2 or 3 if already green
    # Option 3: Trailing Stop / BE lock
    # Option 4: Limit loss per trade / Hard session drawdown shutoff
    options = [
        ("Option A: If in profit, stop if profit >= $4 (Lock any green)", 4.0),
        ("Option B: Lock at $10.00", 10.0),
        ("Option C: Lock at $15.00 with recovery ladder [0.02, 0.05, 0.12, 0.25, 0.40]", 15.0),
    ]

    for opt_name, lock_val in options:
        daily_results = {}
        cur_day = None
        day_pnl = 0.0
        day_trades = []
        ...
        for i in range(25, n - 20):
            ...
            # Check ladder
            ladder = [0.02, 0.05, 0.12, 0.25, 0.40]
            cur_lot = ladder[min(consec_losses, len(ladder)-1)]

            r_low = 28 - (consec_losses * 2)
            r_high = 72 + (consec_losses * 2)
