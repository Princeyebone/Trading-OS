...
def run_zero_loss_master():
    ...
    # Test fine tuning around dev 2.5 - 2.7, rsi 24 - 26, sl_ratio 0.85 - 0.95
    configs = [
        # (bb_dev, rsi_thresh, tp_r, sl_r, lock, ladder, max_consec_scale, max_hour)
        ("Tuned A (Dev 2.55, RSI 25, SL 0.85, Lock 15, Hour 18)", 2.55, 25, 0.70, 0.85, 15.0, [0.03, 0.08, 0.18, 0.28], 18),
        ("Tuned B (Dev 2.60, RSI 25, SL 0.90, Lock 15, Hour 18)", 2.60, 25, 0.70, 0.90, 15.0, [0.03, 0.08, 0.18, 0.28], 18),
        ("Tuned C (Dev 2.50, RSI 24, SL 0.85, Lock 15, Hour 17)", 2.50, 24, 0.70, 0.85, 15.0, [0.03, 0.08, 0.18, 0.28], 17),
        ("Tuned D (Dev 2.65, RSI 26, SL 0.90, Lock 20, Hour 19)", 2.65, 26, 0.75, 0.90, 20.0, [0.03, 0.07, 0.16, 0.25], 19),
        ("Tuned E (Dev 2.50, RSI 25, SL 0.90, Lock 12, Hour 18)", 2.50, 25, 0.70, 0.90, 12.0, [0.03, 0.08, 0.18, 0.28], 18),
    ]

    for label, bb_dev, rsi_thresh, tp_ratio, sl_ratio, lock_target, ladder, max_h in configs:
        ...
        for i in range(25, n - 20):
            ...
            if locked_green or i <= last_exit or not (7 <= cur_hour <= max_h):
                continue
        ...
        print(f"=== {label} ===")
        print(f"Result: {green_days}/{total_days} GREEN ({green_days/total_days*100:.1f}%) | Net PnL: ${tot_pnl:+.2f}")
