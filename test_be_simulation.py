...
def run_be_simulation():
    ...
    # Test with Break-Even scratch: once price moves +50% to target, move SL to Entry + 0.002
    for be_thresh in [0.40, 0.50, 0.60, None]:
        daily_results = {}
        cur_day = None
        day_pnl = 0.0
        day_trades = []
        ...
        for i in range(25, n - 20):
            ...
            if long_c or short_c:
                ...
                orig_sl = entry - (1.2 * (tp - entry)) if is_long else entry + (1.2 * (entry - tp))
                cur_sl = orig_sl
                ...
                for k in range(i, min(i + 20, n)):
                    ...
                    if be_thresh is not None:
                        if is_long and h[k] >= entry + be_thresh * (tp - entry):
                            cur_sl = max(cur_sl, entry + 0.002)
                        elif not is_long and l[k] <= entry - be_thresh * (entry - tp):
                            cur_sl = min(cur_sl, entry - 0.002)

                    if is_long:
                        if l[k] <= cur_sl:
                            pnl = (cur_sl - entry) * mult * cur_lot
                            win = pnl > 0
                            exit_idx = k; break
                        elif h[k] >= tp:
        ...
        print(f"BE THRESHOLD: {be_thresh}")
