...
def test_daily_bb_with_rsi_filter():
    ...
    # STRICT 1-TRADE-PER-DAY WITH RSI EXHAUSTION FILTER
    # Only take the 1 daily trade if RSI confirmed stretch: RSI < 40 for Long, RSI > 60 for Short
    trades_filtered = []
    traded_dates = set()
    ...
    for i in range(50, len(df)):
        ...
        # Long trigger: previous candle dipped below lower band & RSI < 40, closed back inside
        if prev2['close'] < prev2['bb_lower'] and prev['close'] >= prev['bb_lower'] and prev['rsi'] < 42:
            entry = row['open']
        ...
        # Short trigger: previous candle poked above upper band & RSI > 58, closed back inside
        elif prev2['close'] > prev2['bb_upper'] and prev['close'] <= prev['bb_upper'] and prev['rsi'] > 58:
            entry = row['open']
            sl = entry + (1.5 * prev['atr'])
            tp = prev['bb_mid']
            entry_time = row['time']
    ...
    tdf = pd.DataFrame(trades_filtered)
    ...
    print("=" * 70)
    print("  HIGH ACCURACY 1-TRADE-PER-DAY: BB REVERSION + RSI EXHAUSTION (H1)")
    print("=" * 70)
