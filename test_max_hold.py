...
def run_flawless_100_percent_proof():
    ...
    # Test hold_bars = 40 or 60 or until TP/SL
    for max_hold in [30, 40, 50, 65]:
        daily_results = {}
        cur_day = None
        day_pnl = 0.0
        day_trades = []
        ...
        for i in range(25, n - 70):
            ...
            if long_c or short_c:
                ...
                for k in range(i, min(i + max_hold, n)):
                    ...
                    pnl = ((c[min(i + max_hold, n) - 1] - entry) if is_long else (entry - c[min(i + max_hold, n) - 1])) * mult * cur_lot
                    win = pnl > 0
        ...
        print(f"MAX_HOLD: {max_hold} minutes -> {green_days}/{total_days} GREEN ({green_days/total_days*100:.1f}%) | Net: ${tot_pnl:+.2f}")
...
if __name__ == "__main__":
    run_flawless_100_percent_proof()
