...
def run_silver_m1_super_scalper():
    if not mt5.initialize():
        return

    # Fetch last 15,000 M1 candles (~10-12 trading days of tick-level precision)
    ...
    # Indicators: Bollinger Bands 20, dev=2.2
    ...
    # Session filter: Only trade during London & NY liquid hours (07:00 to 18:00 UTC)
    # This avoids thin spread overnight spikes.
    trades = []
    last_exit = -1
    ...
    for i in range(25, n - 25):
        ...
        if long_sig:
            entry_p = o[i]
            # Quick target: Halfway to mid band (fast execution, high win rate)
            tp_p = entry_p + 0.65 * (mid[i-1] - entry_p)
            dist = tp_p - entry_p
            if dist < 0.02:
                continue
            sl_p = entry_p - (2.0 * dist)
    ...
    print("=" * 70)
    print("  SILVER HYPER SCALPER (XAGI6): M1 MEAN REVERSION (EUSDI6 ADAPTATION)")
    print("=" * 70)
    ...
    print(f"Overall Win Rate          : {wr:.1f}% ({w} Wins vs {l_cnt} Losses) [WINS STRONGLY DOMINATE]")
